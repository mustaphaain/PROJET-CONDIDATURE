...
router.get('/stats', async (req, res) => {
    try {
        ...
        const enAttente = await Candidature.countDocuments({ statut: 'Attente' });
        const acceptees = await Candidature.countDocuments({ statut: 'Accepte' });
        const refusees = await Candidature.countDocuments({ statut: 'Refusee' });

        console.log(`Total: ${total}`);
        console.log(`En attente: ${enAttente}`);
        console.log(`Acceptées: ${acceptees}`);
